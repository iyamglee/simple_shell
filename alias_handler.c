...
int print_alias(data_of_program *data, char *alias)
{
	...
	if (data->alias_list)
	{
		...
		for (i = 0; data->alias_list[i]; i++)
		{
			...
			{
				for (m = 0; data->alias_list[i][m]; m++)
				{
					buffer[m] = data->alias_list[i][m];
					if (data->alias_list[i][m] == '=')
						break;
				}
				buffer[m + 1] = '\0';
				buffer_add(buffer, "'");
				buffer_add(buffer, data->alias_list[i] + m + 1);
				buffer_add(buffer, "'\n");
				_print(buffer);
			}
		}
 ...
 * Return: 0 on success, num declared in the args, NULL if not found
 */
